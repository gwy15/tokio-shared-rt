...
impl Configuration {
    ...
    fn set_shared(&mut self, shared: syn::Lit, span: Span) -> Result<(), syn::Error> {
        if self.shared.is_some() {
            return Err(syn::Error::new(span, "`shared` set multiple times."));
        }
        let shared = parse_bool(shared, span, "shared")?;
        self.shared = Some(shared);
        Ok(())
    }
    fn set_shared_raw(&mut self, value: bool) {
        ...
        match arg {
            syn::Meta::NameValue(namevalue) => {
                ...
                match ident.as_str() {
                    ...
                    "shared" => {
                        config.set_shared(lit.clone(), syn::spanned::Spanned::span(lit))?;
                    }
                    name => {
                        let msg = format!(
                            "Unknown attribute {} is specified; expected one of: `flavor`, `worker_threads`, `start_paused`, `crate`, `shared`",
                            name,
                        );
                        return Err(syn::Error::new_spanned(namevalue, msg));
                    }
...
fn parse_knobs(mut input: ItemFn, is_test: bool, config: FinalConfig) -> TokenStream {
    ...
    let last_block = if config.shared == Some(true) {
        quote_spanned! {last_stmt_end_span=>
            ...
            {
                let rt = ::tokio_shared_rt::RUNTIME.get_or_init(|| {
                    #rt
                    .enable_all()
                    .build()
                    .expect("Failed building the Runtime")
                });
                return rt.block_on(#body_ident);
            }
        }
    } else {
        quote_spanned! {last_stmt_end_span=>
